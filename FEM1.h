...
class FEM
{
  public:
    ...
    double                basisFunctionOrder, prob, L, g1, g2, E, A, h, f;    
...
double FEM<dim>::basis_gradient( unsigned int node, double xi )
{
    ...
    // Для численого поиска производной используем 
    // интерполяционный полином Лагранжа для трех узлов (n = 2)
...
void FEM<dim>::setup_system()
{
    ...
    // Выбираем из правила, что квадратура точна для 2n-1 степени многочлена, а мы апроксимируем в случае klocal аппроксимируем N*N и flocal f*N
    quadRule = ceil( ( ( basisFunctionOrder * 2 ) + 1 ) / 2 ); //DONE
    quad_points.resize(quadRule); quad_weight.resize(quadRule);

    // Quad_points stores the value of xi for each quadrature point
    // These are for 2 points
...
void FEM<dim>::assemble_system()
{
    ...
    for ( ; elem != endc; ++elem )
    {
      ...
      { 
        ...
        {
          // DONE P Modify Flocal to include the traction on the right boundary.
          Flocal[1] += h;
        }
      }
      ...
      for( unsigned int A = 0; A < dofs_per_elem; ++A ) 
      {
        ...
        for( unsigned int B = 0; B < dofs_per_elem; ++B ) 
        {
          // DONE P add component A,B of Klocal to the correct location in K (using local_dof_indices)
...
double FEM<dim>::l2norm_of_error()
{
  ...
  for( ; elem != endc; ++ elem )
  {
    ...
    for( unsigned int q = 0; q < quadRule; ++q )
    {
      ...
      //if ( prob == 2 ) u_exact = -x * x * x / ( 6. * E ) + ( h + 0.5 * L * L ) * x + g1;
